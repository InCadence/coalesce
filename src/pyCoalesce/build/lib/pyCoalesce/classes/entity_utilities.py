...
The parse functions were originally auto-generated by :mod:`generateDS`, and
moved from the :mod:`~pyCoalesce.classes.coalesce_entity` module to this one.

Some of the functions don't work properly with iPython's auto-reload feature.
In most cases, these problems can be overcome by restarting the kernel and
reimporting everything at once, though even this workaround may not solve the
...
def parseString(inString, object_class = None, silence = False):
    """
    Parse an XML string, create the object tree, and export it as a Coalesce
    object.
    ...
    :param object_class:  a subclass of any of the classes in
        ...
        :mod:`.pyCoalesce.classes.coalesce_entity`.  This argument allows the
        instantiation of a subclass besides the ones in the latter module.
...
def find_child(Coalesce_object, name, match_case = False,
    ...
    :param include_fielddefinitions:  if ``True``, include
        ...
        in the results.  Normally, we want to exclude these, because they have
        the same names as fields, leading to ambiguity when using this
...
def set_child_attrib(Coalesce_object, path = [u"<root>"], attrib = "value",
    ...
    :param path:  a list of object types and indices that specifies the path
        ...
        presumably only) item in this argument is "<root>", set the attribute
        on the "Coalesce_object" itself.
    :param attrib:  the name of the attribute to be retrieved.  Defaults to
...
def set_entity_fields(Coalesce_entity = None, fields = None, match_case = False):
    ...
    <.pyCoalesce.classes.coalesce_entity.CoalesceField>' with specified values.

    :param Coalesce_entity:  an instance of
        :class:`~pyCoalesce.classes.coalesce_entity.CoalesceEntity` or one of
        its subclasses.
    :param fields:  a dict-like of :class:`fields
        ...
        names (in which case the function searches for each field, and throws
        an error if duplicates are found) or path lists, alternating between
        child object type and list index.  The values of the dict-like must be
        the values to be set on the
        :attr:`~pyCoalesce.classes.coalesce_entity.fieldSub.value` attribute
