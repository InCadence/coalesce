...
public class ElasticSearchIterator extends CoalesceIterator<ElasticSearchIterator.Parameters> {
    ...
    protected boolean visitCoalesceEntity(CoalesceEntity entity, Parameters param) throws CoalesceException
    {
        ...
        source.put(ElasticSearchPersistor.DOC_TYPE,ElasticSearchPersistor.COALESCE_ENTITY);
    ...
    protected boolean visitCoalesceLinkageSection(CoalesceLinkageSection section, Parameters param) throws CoalesceException
    {
        for (CoalesceLinkage linkage : section.getLinkages().values())
        {
            ...
            mapping.put(ElasticSearchPersistor.DOC_TYPE,ElasticSearchPersistor.COALESCE_LINKAGE);
        ...
        if (recordset.isFlatten())
        {
            for (CoalesceRecord record : recordset.getAllRecords())
            {
                if (record.isFlatten())
                {
                    ...
                    mapping.putAll(param.common);
                    mapping.put(ElasticSearchPersistor.DOC_TYPE,"recordset");
    ...
    private Map<String, Object> createPoint(CoalesceCoordinateField field) throws CoalesceDataFormatException
    {
        ...
        if (field.getValue() != null)
        {
            ...
            results = new HashMap<>();
            results.put("type", PointBuilder.TYPE);
    ...
    private Map<String, Object> createPolygon(CoalescePolygonField field) throws CoalesceDataFormatException
    {
        ...
        if (field.getValue() != null)
        {
            ...
            results = new HashMap<>();
            results.put("type", PolygonBuilder.TYPE);
...
//            line.put(getCoordinates(field.getValue().getCoordinates()));

            results = new HashMap<>();
            results.put("type", LineStringBuilder.TYPE);
    ...
    private Map<String, Object> createCircle(CoalesceCircleField field) throws CoalesceDataFormatException
    {
        ...
        if (circle != null)
        {
            ...
            shapeFactory.setCentre(new org.locationtech.jts.geom.Coordinate(circle.getCenter().x,circle.getCenter().y));
            shapeFactory.setSize(circle.getRadius());
            Polygon polyCircle = shapeFactory.createCircle();
            jsonArray.put(getLocationtechCoordinates((polyCircle.getCoordinates())));

            results = new HashMap<>();
            results.put("type", PolygonBuilder.TYPE);
            results.put("coordinates", jsonArray);
...
//
//            results = new HashMap<>();
//            results.put("type", CircleBuilder.TYPE);
...
//            results.put("radius", circle.getRadius() + "m");
    ...
    private Map<String, Object> createMultiPoint(CoalesceCoordinateListField field) throws CoalesceDataFormatException
    {
        ...
        {
            results = new HashMap<>();
            results.put("type", MultiPointBuilder.TYPE);
    ...
    private JSONArray getLocationtechCoordinates(org.locationtech.jts.geom.Coordinate[] coords)
    {
        JSONArray results = null;
