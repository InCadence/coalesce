    ...
    public Marking(String markingString)
    {
        ...
        else if (parts.length > 1)
        {
            ...
            if (parts[index].startsWith("JOINT"))
            {
                ...
                if ((joint.contains("TOP") && joint.size() < 5) || joint.size() < 4) return; // Invalid Joint Classification
                                                                                             // marking

                if (joint.contains("TOP"))
                {
                    marking = "JOINT TOP SECRET";
                }
                else
                {
                    marking = "JOINT " + joint.get(1);
                }
            ...
            else
            {
                ...
                if ((fgc.contains("TOP") && fgc.size() < 3) || fgc.size() < 2) return; // Invalid FGC Classification marking
                ...
                else
                {
                    if (fgc.contains("TOP"))
                    {
                        _classification = FieldValues.getMarkingValueByTitle("TOP SECRET", GetClassifications());
                    }
                    else
                    {
                        _classification = FieldValues.getMarkingValueByTitle(fgc.get(1), GetClassifications());
