...
public class DerbyPersistor extends CoalescePersistorBase implements
		ICoalesceSearchPersistor {

	/*--------------------------------------------------------------------------
	Private Members
	--------------------------------------------------------------------------*/
	private static final Logger LOGGER = LoggerFactory
			.getLogger(DerbyPersistor.class);
	...
	protected CoalesceDataConnectorBase getDataConnector()
		...
		// }
		// TODO Re-look this.
		return new DerbyDataConnector(getConnectionSettings().getDatabase(),
				this.getSchema(), "memory");
	}
	...
	protected boolean persistEntityObject(CoalesceEntity entity,
		...
		return ((DerbyDataConnector) conn).coalesceEntity_InsertOrUpdate(
				entity.getKey(), entity.getName(), entity.getSource(),
				entity.getVersion(), entity.getEntityId(),
				entity.getEntityIdType(), entity.toXml(),
				entity.getDateCreated(), entity.getLastModified());
	}
	...
	protected boolean persistRecordsetObject(CoalesceRecordset recordset,
		...
		if (conn instanceof DerbyDataConnector) {
			...
			throw new SQLException(
					"Derby Persistor requires the DerbyDataConnector.");
		}
		...
		if (tableExists(info, getSchema(), derbyConn)) {
				...
				for (CoalesceRecord record : recordset.getAllRecords()) {
					...
					if (record.isActive()) {
						...
						parameters.add(new CoalesceParameter("objectkey",
								record.getKey(), ECoalesceFieldDataTypes.STRING_TYPE.ordinal()));
						parameters.add(new CoalesceParameter("entitykey", info
								.getEntity().getKey(), ECoalesceFieldDataTypes.STRING_TYPE.ordinal()));
						parameters.add(new CoalesceParameter("entityname", info
								.getEntity().getName(), ECoalesceFieldDataTypes.STRING_TYPE.ordinal()));
						parameters.add(new CoalesceParameter("entitysource",
								info.getEntity().getSource(), ECoalesceFieldDataTypes.STRING_TYPE.ordinal()));
						parameters.add(new CoalesceParameter("entitytype", info
								.getEntity().getVersion(), ECoalesceFieldDataTypes.STRING_TYPE.ordinal()));
						...
						for (String column : derbyConn.getColumnNames(
								getSchema(), info.getTableName())) {

							String value;

							CoalesceField<?> field = record
									.getFieldByName(column);

							// Field Found?
							if (field != null) {
								// Can flatten Field?
								if (field.getFlatten()
										&& !StringHelper.isNullOrEmpty(field
												.getBaseValue())) {
								...
								if (value != null) {
								parameters.add(new CoalesceParameter(column, value,
										field.getDataType().ordinal()));
								}
							...
							/*
								...
								// No; Pass null
								parameters.add(new CoalesceParameter(null,
										Types.OTHER));
							}
							*/
						}
						...
						try {
							if (!derbyConn.insertRecord(getSchema(),
									info.getTableName(), parameters)) {
								LOGGER.error("FAILED to insert Record for "
										+ getSchema() + "."
										+ info.getTableName());
							}
						} catch (CoalesceException ce) {
							LOGGER.error("FAILED to insert Record for "
									+ getSchema() + "." + info.getTableName() + ", REASON: " + ce.getCause().getLocalizedMessage());
						}
			...
			LOGGER.warn(String.format("Object Not Registered (%s : %s)", info
					.getEntity().getName(), recordset.getName()));
		}
	...
	protected boolean persistLinkageObject(CoalesceLinkage linkage,
		...
		// Yes; Call Procedure
		return ((DerbyDataConnector) conn)
				.coalesceLinkage_InsertOrUpdate(linkage);
	}
	...
	public static boolean tableExists(final CoalesceIndexInfo info,
			final String schema, final DerbyDataConnector conn)
			throws SQLException {
	...
	public void saveTemplate(CoalesceDataConnectorBase conn,
		...
		try {
			for (CoalesceEntityTemplate template : templates) {
				((DerbyDataConnector) conn)
						.coalesceEntityTemplate_InsertOrUpdate(template);
			}
		} catch (Exception e) {
	...
	public SearchResults search(Query query) throws CoalescePersistorException {
		...
		try {
			// Create SQL Query
			DerbyCoalescePreparedFilter preparedFilter = new DerbyCoalescePreparedFilter(
					DerbySettings.getDatabaseSchema());
			...
			preparedFilter.setFeatureType(CoalesceFeatureTypeFactory
			...
			CoalesceParameter[] params = paramList
					.toArray(new CoalesceParameter[paramList.size()]);

			try (CoalesceDataConnectorBase conn = new DerbyDataConnector(
					getConnectionSettings().getDatabase(), this.getSchema(),
					"memory")) {
				String sql = String.format("SELECT DISTINCT %s FROM %s %s %s",
						preparedFilter.getColumns(), preparedFilter.getFrom(),
						where, preparedFilter.getSorting());

				// Get Hits
				CachedRowSet hits = RowSetProvider.newFactory()
						.createCachedRowSet();
				...
				if (numberOfHits >= query.getMaxFeatures()) {
					// Yes; Get Total Hits
					sql = String.format("SELECT DISTINCT COUNT(*) FROM %s %s",
							preparedFilter.getFrom(), where);
		...
		} catch (FilterToSQLException | SQLException | ParseException
				| CoalesceException e1) {
	...
	private List<CoalesceParameter> getParameters(
			DerbyCoalescePreparedFilter filter) throws ParseException {

		List<CoalesceParameter> parameters = new ArrayList<CoalesceParameter>();

		// Add Parameters
		for (Object value : filter.getLiteralValues()) {
			parameters.add(new CoalesceParameter(value.toString(),
					Types.VARCHAR));
		}
