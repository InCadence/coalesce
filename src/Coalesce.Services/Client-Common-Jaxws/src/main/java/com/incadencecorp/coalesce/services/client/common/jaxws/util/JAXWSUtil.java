...
public final class JAXWSUtil {
    ...
    public static <T> T createClient(Class<T> clazz, URL url, Bus bus)
    {
        ...
        {
            LOGGER.debug("Creating {} Client (url = {})",
                         LOGGER.isTraceEnabled() ? clazz.getName() : clazz.getSimpleName(),
                         url);
        }
            ...
            throws WSSecurityException, ParserConfigurationException
    {
        ...
        Element usernameTokenElement = doc.createElementNS(
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd",
                "UsernameToken");
        Element usernameElement = doc.createElementNS(
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd",
                "Username");
     ...
     * container it will use the path specified by @{link {@link JAXWSUtilSettings#getBusConfiguration()}
     * to create the Bus from a Spring configuration file.
     *
    ...
    public static Bus getBus()
    {
        ...
        if (bus == null)
        {
            Path configuration = JAXWSUtilSettings.getBusConfiguration();

            // No; Create from OSGI
            try
            {
                bus = getBusFromOSGI(configuration);
            }
            ...
            if (bus == null)
            {
                ...
                bus = getBusFromConfiguration(configuration);
            }
     ...
     */
    private static Bus getBusFromOSGI(Path configuration) throws InvalidSyntaxException
    {
        Bus bus = null;
        ...
        if (bundle != null)
        {
            ...
            if (refs != null && refs.length > 0)
            {
                ...
                case 1:
                    ...
                    String configName = getConfigName(configuration);
    ...
    private static Bus getBusFromConfiguration(Path configuration)
    {
        ...
        if (SPRING_VALIDATION)
        {
            ...
            try
            {
                bus = bf.createBus(configuration.toString());
            }
            catch (Exception e)
            {
                bus = bf.createBus();
        ...
        else
        {
            ...
            context.load(configuration.toString());

            XmlBeanDefinitionReader reader = new XmlBeanDefinitionReader(context);
            reader.setValidationMode(XmlBeanDefinitionReader.VALIDATION_NONE);
            reader.loadBeanDefinitions(configuration.toString());
            context.refresh();
