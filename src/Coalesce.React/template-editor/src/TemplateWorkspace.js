...
class TemplateWorkspace extends Component {
  constructor(props) {
    ...
    this.handleDuplicateTemplateLoad = this.handleDuplicateTemplateLoad.bind(
      this
    );
...
//*/
  ...
  handleTemplateRegister() {
    ...
    if (items.length > 0) {
      ...
      items.forEach(function(item) {
        registerTemplate(item.template.key)
          .then(function(result) {
            if (++count >= items.length) {
              that.setState({
                loading: null
  ...
  debugItemPrint() {
    for (var ii = 0; ii < this.state.items.length; ii++) {
      console.log(
        ...
        ii,
        "X is",
        this.state.items[ii].x,
        "Y is",
        this.state.items[ii].y,
        "W is",
        this.state.items[ii].w,
        "H is",
        this.state.items[ii].h,
        "Key is",
        this.state.items[ii].template.key
      );
    }
  }
    ...
    return (
      <div className="App">
        ...
        <div>
          ...
          {this.state.loading && (
            <DialogLoader title={this.state.loading} opened={true} />
          )}
          {duplicates && duplicates.length > 0 && (
            <DialogMessage
              ...
              message={`${
                duplicates.length
              } templates with matching key exists.  Continuing(OK) will overwrite any unsaved changes`}
              onClose={() => {
